...
mpl.rcParams["agg.path.chunksize"]=40000; #Matplotlibrc file set at 20000. High enough for most scripts.
...
plt.scatter(minTime,minDist, c=protTime, label="Gradient_6nm_v1") 
...
plt.title("Protein Position over Time (In Contact Only)")
plt.colorbar(); plt.legend(); 
plt.show(); plt.savefig("graphs/ProtPos/G1.1_Protein_InContact_Position.png")
#GRAPH 1.2:
...
plt.colorbar();
plt.show(); plt.savefig("graphs/ProtPos/G1.2_Protein_AllData_Position.png")
...
plt.title("Protein Position Heat Map (In Contact Only)")
plt.colorbar();
...
plt.xlim(0, limitX);
plt.show(); plt.savefig("graphs/ProtPos/G2_Frequency_Against_Radial_Distance.png")
...
plt.show()
plt.savefig("graphs/ProtPos/G5_2D_Heat_Map.png")
...
plt.scatter(sparseX,sparseY, alpha=0.7, c=sparseT, marker=".", edgecolors='none')
plt.colorbar();
plt.title("Protein Movements over time (plot every %i frames)" % plotEvery);
plt.show()
plt.savefig("graphs/ProtPos/G6_Time_AND_G5.png")
